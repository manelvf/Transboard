...
class TransboardTest < Test::Unit::TestCase
  ...
  def test_login
    u = $db['mm_users'].find({:email=>"manelvf@gmail.com"})
    assert_equal u.to_a.length, 1
    ...
    #assert last_request.url.gsub("dashboard")
    #assert last_response.ok?
  ...
  def test_upload_form_is_ok
    ...
    #assert last_response.body.include?('strings loaded')
  end
